...
export default function AdminOrdersScreen({ navigation }: AdminOrdersScreenProps) {
  ...
  const renderOrderList = (orderList: OrderData[], showDelete: boolean = true) => (
    orderList.map((order) => {
        ...
        return (
          <SwipeableOrderCard
            ...
            onDelete={showDelete ? () => handleDeleteOrder(order.id!, order.customerName) : undefined}
          />
        )
    })
  );
        ...
        >
          {activeMainTab === 'active' ? (
            filteredActiveOrders.length > 0 ? (
              renderOrderList(filteredActiveOrders, true)
            ) : (
              <View style={styles.emptyState}>
                <Text style={styles.emptyStateText}>No active orders found</Text>
                <Text style={styles.emptyStateSubtext}>All orders are completed or no orders match your filters</Text>
              </View>
            )
          ) : (
            ...
            <View style={[styles.modalContainer, {paddingBottom: insets.bottom}]}>
                ...
                {selectedOrder && (
                    <ScrollView>
                        ...
                        {/* Show current delivery agent if assigned */}
                        {selectedOrder.deliveryAgentName && selectedOrder.id !== 'multiple' && (
                            <View style={styles.modalSection}>
                                <Text style={styles.modalSectionTitle}>Current Delivery Agent</Text>
                                <View style={styles.currentAgentRow}>
                                    <View style={{flexDirection: 'row', alignItems: 'center', gap: 12}}>
                                        <User size={18} color={Colors.primary} />
                                        <Text style={[styles.agentName, {color: Colors.primary}]}>{selectedOrder.deliveryAgentName}</Text>
                                    </View>
                                    <View style={styles.currentAgentBadge}>
                                        <Text style={styles.currentAgentBadgeText}>Currently Assigned</Text>
                                    </View>
                                </View>
                            </View>
                        )}
                          ...
                          selectedOrder.id === 'multiple') && (
                            <View style={styles.modalSection}>
                                <Text style={styles.modalSectionTitle}>
                                    {selectedOrder.id === 'multiple' 
                                      ? `Assign Agent to ${selectedOrders.length} Orders` 
                                ...
                                {deliveryAgents.map(agent => (
                                    <TouchableOpacity key={agent.id} style={styles.agentRow} onPress={() => setSelectedAgent(agent.id || null)}>
                                        <View style={{flexDirection: 'row', alignItems: 'center', gap: 12}}>
                                            ...
                                            {selectedOrder.deliveryAgentId === agent.id && (
                                                <View style={styles.currentAgentIndicator}>
                                                    <Text style={styles.currentAgentIndicatorText}>Current</Text>
                                                </View>
                                            )}
                                        </View>
...
const styles = StyleSheet.create({
  ...
  ordersList: {
    padding: 20,
    paddingBottom: 80, // Extra padding to account for bulk assign footer
  },
  sectionTitle: {
    fontSize: 18,
    fontFamily: 'Inter_600SemiBold',
  ...
  currentAgentIndicatorText: {
    color: Colors.primary,
    fontSize: 10,
    fontFamily: 'Inter_500Medium',
  },
  // Order card delivery info styles
  deliveryInfo: {
    flexDirection: 'row',
    alignItems: 'center',
    gap: 8,
