...
export const subAdminAuthService = {
  // Authenticate sub-admin or admin with email and password
  async authenticateSubAdmin(email: string, password: string): Promise<UserSession | null> {
    try {
      // Query the unified users collection for sub-admin or admin role
      const usersQuery = query(
        collection(FIREBASE_DB, 'users'),
        where('email', '==', email),
        where('role', 'in', ['sub-admin', 'admin'])
      );
      ...
      // If role is 'admin', return an admin session (full access), skip sub-admin permissions lookup
      ...
      // For sub-admin, fetch permissions document
  ...
  // Authenticate sub-admin or admin with phone number and password
  async authenticateSubAdminByPhone(phone: string, password: string): Promise<UserSession | null> {
    try {
      // Query the unified users collection for sub-admin or admin role with phone number
      const usersQuery = query(
        collection(FIREBASE_DB, 'users'),
      ...
      // If role is 'admin', return an admin session and skip permissions lookup
      if (userData.role === 'admin') {
        const adminSession: UserSession = {
          uid: userDoc.id,
      ...
      // For sub-admin, fetch permissions
      const subAdminData = await subAdminService.getSubAdminById(userDoc.id);
      if (!subAdminData) {
        throw new Error('Sub-admin data not found');
      }
